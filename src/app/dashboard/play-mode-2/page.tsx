        ...
        <div className="border p-6 rounded-lg md:w-1/2 w-full">
          ...
          {index <= 9 && (
            <div className="w-full grid md:grid-cols-2 mt-6 gap-6">
              {data[index].incorrect_answers.map((item: string) => (
				<button className="py-4 px-6 bg-slate-100 rounded-lg text-md hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out hover:scale-105 active:scale-100" onClick={() => checkAnswer(item)} key={item}>{item}</button>
			  ))}
			 <button className="py-4 px-6 bg-slate-100 rounded-lg text-md hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out hover:scale-105 active:scale-100" onClick={() => checkAnswer(data[index].correct_answer)}>{data[index].correct_answer}</button>
            </div>
          )}
        </div>
      ) : (
